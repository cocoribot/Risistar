...
<tr>
	...
	{if $Info.state == 1}
		...
		<td><a href="?page=module&amp;mode=deaktiv&amp;id={$ID}&amp;sid={$sid|escape}">{$mod_change_deactive}</a></td>
	{else}
		<td style="color:red"><b>{$mod_deactive}</b></td>
		<td><a href="?page=module&amp;mode=aktiv&amp;id={$ID}&amp;sid={$sid|escape}">{$mod_change_active}</a></td>
	{/if}
	</tr>
{/foreach}
</table>
